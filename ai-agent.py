...
def _process_response(db, response: str) -> str:
        ...
        # 根据AI返回的带SQL的数据，进行匹配，过滤出查找SQL代码块
        ...
        #clean_response = clean_response.replace("查询表数据", "")
        #clean_response = clean_response.replace("查询表结构", "")
...
# 初始化对话历史列表，可以先包含一个 system 消息设定角色
...
with it.sidebar:
    ...
    if it.button("新建会话", width="stretch", icon="✏"):
        ...
        it.session_state.current_session=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        it.session_state.message = []
        it.session_state.conversation_history = [{"role": "system", "content": ""}]  # 重置历史
        nature = "可爱"
        nick_name = "小可爱"
        it.rerun()
    it.text("会话历史")
...
if prompt:
  client = OpenAI(
    api_key= "sk-e4a696a7a99447e8a607346d6c718826" ,
    base_url="https://api.deepseek.com"
  )
  table_str_escaped = table_str.replace('%', '%%')
  c="你是一个%s女生，你的名字叫%s，不能在回答中有删除线，同时你也是数据分析师,如果是查询数据，生成“查询表数据”，帮我生成1条查询的SQL语句，查询返回字段用中文。如果列出或查询表结构，则不返回SQL;我现在数据库的表结构有:"+table_str_escaped
  system_content = c.format(nature, nick_name)
  # 构建 messages（注意：Streamlit 的 history 就是 API 需要的格式）
  api_messages = [
                     {"role": "system", "content": system_content}
                 ] + it.session_state.message  # 直接使用已保存的历史
  ...
  if judge_select(full_response):
    ...
    conversation_history.append({"role": "user", "content": outputStr})
